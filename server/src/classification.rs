...
pub fn _add_classification(_req: &ClassificationRequest) -> Result<String, String> {
    // Validate category
    ...
    // Create entry
    let _entry = ClassificationEntry {
        id: _id.clone(),
    ...
    if let Some(_entries) = _json["entries"].as_array_mut() {
        _entries.push(serde_json::to_value(&_entry)
...
pub fn _undo_classification(_category: &str, _address: &str) -> Result<String, String> {
    ...
    if let Some(_entries) = _json["entries"].as_array_mut() {
        ...
        if let Some(_last_index) = _entries.iter().rposition(|e| {
            ...
            return Ok(format!("Undid last classification for '{}' in category '{}'", _address, _category));
        }
    }

    Err(format!("No classifications found for '{}' in category '{}'", _address, _category))
}
