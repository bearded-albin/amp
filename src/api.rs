...
impl ArcGISClient {
    ...
    async fn fetch_all_features(
        ...
        loop {
            ...
            println!("Received {} features", response.features.len());
        ...
        println!("Total features collected: {}", all_features.len());
        Ok(all_features)
    }
            ...
            if let Some(points) = points_array.as_array() {
                if let Some(first_point) = points.first() {
                    if let Some(coords) = first_point.as_array() {
                        if coords.len() >= 2 {
                            let x = Decimal::from_f64_retain(coords[0].as_f64()?)
                                .unwrap_or_default();
                            let y = Decimal::from_f64_retain(coords[1].as_f64()?)
                                .unwrap_or_default();
                            return Some([x, y]);
                        }
                    }
                ...
                if !paths_array.is_empty() {
                    if let Some(first_path) = paths_array[0].as_array() {
                        if first_path.len() >= 2 {
                            ...
                            let first_x = Decimal::from_f64_retain(first_pt[0].as_f64()?)
                                .unwrap_or_default();
                            let first_y = Decimal::from_f64_retain(first_pt[1].as_f64()?)
                                .unwrap_or_default();

                            // Get last point
                            let last_pt = &first_path[first_path.len() - 1];
                            let last_x = Decimal::from_f64_retain(last_pt[0].as_f64()?)
                                .unwrap_or_default();
                            let last_y = Decimal::from_f64_retain(last_pt[1].as_f64()?)
                                .unwrap_or_default();
