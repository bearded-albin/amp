...
pub fn correlation(points: Vec<AdressClean>, lines: Vec<MiljoeDataClean>) -> Vec<AdressInfo> {
    println!("Starting correlation analysis...");
    println!("Processing {} addresses against {} parking zones", points.len(), lines.len());
    ...
    let mut dist_samples = vec![];
    let mut relevant_count = 0;
    let mut irrelevant_count = 0;
    let mut no_closest_line = 0;
    ...
    for (i, res) in results.iter().enumerate() {
        match res {
            ...
            None => {
                no_closest_line += 1;
                println!("Point {} has no closest line", i);
            }
        }
    }

    // Print correlation statistics
    println!("\nCorrelation stats - Relevant (≤50m): {}, Irrelevant (>50m): {}, Errors: {}",
             relevant_count, irrelevant_count, no_closest_line);
    ...
    println!("Correlation analysis complete: {} results generated", correlation.len());
...
pub fn find_closest_lines(
    ...
    println!("Computing closest parking zones for {} addresses...", points.len());
    ...
    println!("Distance computation complete");
    results
}